...
class MutualConsentPolicy(models.Model):
    _name = 'bhu.mutual.consent.policy'
    _description = 'Mutual Consent Purchase Policy / आपसी सहमति की क्रय नीति'
    ...
    # Mutual Consent Policy specific fields
    agreement_date = fields.Date(string='Agreement Date / समझौता दिनांक', tracking=True)
    agreement_number = fields.Char(string='Agreement Number / समझौता संख्या', tracking=True)
    
    # Applicable districts (Raigarh and Pasaur only)
    district_id = fields.Many2one('bhu.district', string='District / जिला', tracking=True,
                                 domain="[('name', 'ilike', 'Raigarh'), ('name', 'ilike', 'Pasaur')]",
                                 help='Only applicable in Raigarh and Pasaur districts')
    
    # Landowner details
    landowner_name = fields.Char(string='Landowner Name / भूमिस्वामी का नाम', tracking=True)
    landowner_consent = fields.Boolean(string='Landowner Consent / भूमिस्वामी की सहमति', default=False, tracking=True)
    consent_date = fields.Date(string='Consent Date / सहमति दिनांक', tracking=True)
    ...
    agreement_filename = fields.Char(string='Agreement Filename', tracking=True)
    consent_file = fields.Binary(string='Consent File / सहमति फ़ाइल', tracking=True)
    consent_filename = fields.Char(string='Consent Filename', tracking=True)
        ...
        for vals in vals_list:
            if vals.get('name', 'New') == 'New':
                ...
                if project_id:
                    ...
                    sequence_code = 'bhu.mutual.consent.policy'
                    if project:
                        sequence_code = f'bhu.mutual.consent.policy.{project.id}'
