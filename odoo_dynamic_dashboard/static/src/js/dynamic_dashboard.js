...
export class OdooDynamicDashboard extends Component {
    ...
    setup() {
        ...
        onWillStart(async () => {
                ...
                await this.orm.call('dashboard.block', 'get_action_id', [[]]).then((response) => {
                    this.main_action_id = response;
                });
                await this.orm.call('dashboard.block', 'get_current_action_kpis', [[],this.actionId]).then((response) => {
                    this.state.kpis = response;
                });
    ...
    async onClickAddKPI(event){
        await this.orm.call('dashboard.block', 'get_pop_up_tree_view_id', [[]]).then((response) => {
                    this.action_pop_up_tree = response;
                });
        ...
        this.action.doAction({
            ...
            views: [[this.action_pop_up_tree, 'list']],
            domain: [['client_action_id', '=', this.actionId]],
            target: 'new', 
            context: {
                show_select_footer: true, 
                create: false,
    ...
    /* Function for resizing and dragging the div resize-drag */
        $('.items .resize-drag').each(function(index, element) {
            ...
            function dragMoveListener (event) {
                ...
                var newLeft = currentLeft + event.dx;
                var newTop = currentTop + event.dy;
    ...
    /* Function for rendering the dashboard */
        ...
        await this.orm.call("dashboard.block", "get_dashboard_vals", [[], this.actionId]).then( async (response)=>{
            console.log("response",response)
            var prevLength = Number(localStorage.getItem("prev_length"+this.actionId)) || 0;
            console.log("prevLength",prevLength)
            var newly_added_element = 0
            var highestTop = 0;
            if (response.length > prevLength && prevLength != 0){
                newly_added_element = response.length - prevLength
                
            }
            localStorage.setItem("prev_length"+this.actionId, response.length);
            for (let i = 0; i < response.length; i++) {
               
                if (i >= (response.length - newly_added_element)) {
                    console.log("only run when new element is added after first time", Number(localStorage.getItem("highest_top")));

                    const new_top = Number(localStorage.getItem("highest_top")) || 0;
                
                    const new_element_top_value = new_top  + 20;

                    const new_element_top = new_element_top_value + 'px';
                    response[i].translate_x = new_element_top;

                    // Store numeric value only (no px)
                    localStorage.setItem("highest_top", new_element_top_value);

                    await self.orm.call("dashboard.block", "set_dashboard_vals", [[], response[i].id, response[i].translate_x]);
                }


                if (i!=0 && !response[i].translate_x  && i < (response.length-newly_added_element)){
                    console.log("run only first time and i not zero",i,(response.length-newly_added_element))
                  
                    
                    const currentTop = parseFloat(response[i-1].translate_x) || 0;
                    const height = parseFloat(response[i-1].height) || 0;
                    const spacing = 20;

                    const top = currentTop + height + spacing + 'px';
                    response[i].translate_x = top;
                    await self.orm.call("dashboard.block", "set_dashboard_vals", [[],response[i].id,response[i].translate_x])
                }

                const heightElement = parseFloat(response[i].height) || 0;
                const topElement = parseFloat(response[i].translate_x) || 0; 
                var elementBottom = topElement + heightElement;
                if (elementBottom > highestTop){
                    // Here we are getting the highest top where the new element should be placed 
                    highestTop = elementBottom; 
                    
                }
                
                if (response[i].type === 'tile'){
                    var dashboard = DynamicDashboardTile
                    
                }
                else{
                    var dashboard = DynamicDashboardChart
                }
                mount(dashboard,$('.items')[0], { props: {
                    widget: response[i], doAction: self.action, rpc: self.rpc, dialog:self.dialog, orm: self.orm
                }});
                localStorage.setItem("highest_top", highestTop);
            }
            // localStorage.setItem("last_element", JSON.stringify(last_element));
