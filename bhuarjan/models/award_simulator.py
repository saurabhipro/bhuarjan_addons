...
class AwardSimulator(models.Model):
    ...
    # पुनर्वास नीति: 1 ha = 2.471 acre; ₹/acre — पड़ती 6L, असिंचित 8L, सिंचित 10L
    ...
    def _is_fallow_survey(self, survey):
        """पड़ती भूमि — same rule as Section 23 award."""
    ...
    def _rehab_policy_minimum_rupees(self, acquired_ha, is_fallow, irrigation_type):
        """ha × 2.471 acre × ₹/acre: पड़ती 6L, असिंचित 8L, सिंचित 10L (fallow overrides irrigation)."""
        self.ensure_one()
        acres = (acquired_ha or 0.0) * self.HECTARES_TO_ACRES
        if is_fallow:
            rate = 600_000.0
        elif (irrigation_type or 'unirrigated') == 'irrigated':
            rate = 1_000_000.0
        else:
            rate = 800_000.0
    ...
    def get_land_compensation_data(self):
        ...
        if not surveys:
            ...
            it = self.irrigation_type or 'unirrigated'
            rehab = self._rehab_policy_minimum_rupees(self.acquired_area, False, it)
            payable = max(self.land_total, rehab)
            return [{
                ...
                'unirrigated': it == 'unirrigated',
                'irrigated': it == 'irrigated',
                'is_fallow': False,
                'is_diverted': False,
                ...
                'rehab_policy_amount': rehab,
                'paid_compensation': payable,
        ...
        for survey in surveys:
            ...
            # Always derive Main Road/BMR from measured distance.
            ...
            is_within_distance = distance_from_main_road <= threshold
            road_type = 'mr' if is_within_distance else 'mbr'
            ...
            rehab_for_survey = self._rehab_policy_minimum_rupees(acquired_area, is_fallow, irrigation_type)
            ...
            if not landowners:
                ...
                if key not in compensation_data:
                    compensation_data[key] = {
                        ...
                        'unirrigated': (not is_fallow) and irrigation_type == 'unirrigated',
                        'irrigated': (not is_fallow) and is_irrigated,
                        'is_fallow': is_fallow,
                ...
                compensation_data[key]['rehab_policy_amount'] += rehab_for_survey
                compensation_data[key]['paid_compensation'] = max(
                    compensation_data[key]['total_compensation'],
                    compensation_data[key]['rehab_policy_amount'],
                )
            else:
                for landowner in landowners:
                    ...
                    compensation_data[key]['rehab_policy_amount'] += rehab_for_survey * share
                    compensation_data[key]['paid_compensation'] = max(
                        compensation_data[key]['total_compensation'],
                        compensation_data[key]['rehab_policy_amount'],
                    )
    ...
    def get_tree_compensation_data(self):
        ...
        def _survey_land_area_ha(survey):
            if not survey:
                return 0.0
            return (survey.acquired_area or 0.0) or (survey.total_area or 0.0)

        manual_tree_data = []
        for t_line in self.tree_line_ids:
            base = t_line.line_total or 0.0
            manual_tree_data.append({
                'landowner': None, 'landowner_name': 'Manual / मैनुअल', 'father_name': '',
                'khasra': 'N/A', 'total_khasra': 'N/A', 'total_area': 0.0,
                'land_khasra': 'N/A', 'land_area_ha': 0.0, 'tree_khasra': 'N/A',
                'mulya': base, 'kul_rashi': base,
            ...
            if not landowners:
                for tree_line in tree_lines:
                    ...
                    if key not in tree_data:
                        la = _survey_land_area_ha(survey)
                        tree_data[key] = {
                            'landowner': None, 'landowner_name': '', 'father_name': '',
                            'khasra': khasra, 'total_khasra': '', 'total_area': la,
                            'land_khasra': khasra, 'land_area_ha': la, 'tree_khasra': khasra,
                            'mulya': 0.0, 'kul_rashi': 0.0,
    ...
    def action_online_transfer(self):
        """Send grouped beneficiary payouts to IndusInd online authorization API."""
        self.ensure_one()
        import requests
        ...
        transactions = []
        for group in self.get_land_compensation_grouped_data():
            lines = group.get('lines', [])
            owners = [line.get('landowner') for line in lines if line.get('landowner')]
            if not owners:
                continue
            landowner = owners[0]
            amount = float(group.get('paid_compensation') or group.get('total_compensation') or 0.0)
            unique_khasras = sorted({(line.get('khasra') or '').strip() for line in lines if line.get('khasra')}, key=_khasra_sort_key)
            transactions.append({
                'beneficiary_name': group.get('landowner_name') or landowner.name or '',
                'bank_name': landowner.bank_name or '',
                'account_number': landowner.account_number or '',
                'ifsc_code': landowner.ifsc_code or '',
                'khasra': ', '.join(unique_khasras),
                'amount': round(amount, 2),
            })

        if not transactions:
            raise ValidationError(_('No landowner payment data found for online transfer.'))
        ...
        try:
            ...
            raise ValidationError(_('Failed to connect to IndusInd API: %s') % str(e))

        if response.status_code >= 400:
            raise ValidationError(
                _('IndusInd API error (%s): %s') % (response.status_code, (response.text or '')[:500])
            )

        response_message = 'Online authorization request submitted.'
        try:
            body = response.json()
            response_message = body.get('message') or body.get('status') or response_message
        except Exception:
            if response.text:
    ...
    def action_download_payment_file_excel(self):
        """Download bank payment file in existing payment-file Excel format."""
        self.ensure_one()
        import io
        import base64
        try:
        ...
        if not payment_rows:
            raise ValidationError(_('No landowner payment data found to generate payment file.'))

        debit_account = ''
        settings_model = self.env['bhuarjan.settings.master']
        if hasattr(settings_model, 'get_settings_master'):
            settings = settings_model.get_settings_master()
            debit_account = settings.debit_account_number or ''
        ...
        external_ref = f"SIM-{self.id}"
        for idx, row in enumerate(payment_rows, start=1):
            worksheet.write(idx, 0, 'N', cell_format)
            worksheet.write(idx, 1, external_ref, cell_format)
            worksheet.write(idx, 2, debit_account, cell_format)
            worksheet.write_number(idx, 3, row['amount'], amount_format)
            worksheet.write(idx, 4, row['beneficiary_name'], cell_format)
            worksheet.write(idx, 5, row['bank_name'], cell_format)
            worksheet.write(idx, 6, row['account_number'], cell_format)
            worksheet.write(idx, 7, row['ifsc_code'], cell_format)
            worksheet.write(idx, 8, self.village_id.name or '', cell_format)
            worksheet.write(idx, 9, self.project_id.name or '', cell_format)
            worksheet.write(idx, 10, '', cell_format)
            worksheet.write(idx, 11, self.project_id.name or '', cell_format)
            worksheet.write(idx, 12, self.village_id.name or '', cell_format)
            worksheet.write(idx, 13, row['khasra'], cell_format)
        ...
        filename = f"Payment_File_AwardSimulator_{self.village_id.name or 'Unknown'}.xlsx"
    ...
    def get_award_village_scope_summary(self):
        """Totals for village-summary export (PDF/Excel)."""
        self.ensure_one()
        v = self.village_id
        return {
    ...
    def _action_download_excel_village_only(self):
        ...
        row = 0
        sheet.merge_range(row, 0, row, 2, 'VILLAGE AWARD SUMMARY / ग्राम अवार्ड सारांश', title_fmt)
        row += 2
        rows = [
        ...
        output.close()
        attachment = self.env['ir.attachment'].create({
            'name': f"Award_Village_Summary_{self.village_id.name or 'Manual'}.xlsx",
            'type': 'binary',
            'datas': file_data,
            'mimetype': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
        })
            ...
            if not land_groups:
                ...
                for i, group in enumerate(land_groups, 1):
                    ...
                    for idx, land in enumerate(lines):
                        ...
                        is_fallow = bool(land.get('is_fallow'))
                        ...
                        if is_within_distance:
                            # Main-road khasra: sub-columns 8/9/10 are not applicable.
                            ...
                            if is_fallow:
                                land_sheet.write(row, 7, 'पड़ती', cell_center_fmt)
                                land_sheet.write(row, 8, '—', cell_center_fmt)
                                land_sheet.write(row, 9, '—', cell_center_fmt)
                            else:
...
class AwardSimulatorLandLine(models.Model):
    ...
    def action_edit_land_attributes(self):
        """Open a dialog to edit survey-backed distance, irrigation, diverted, and MR/BMR band."""
        self.ensure_one()
        if not self.survey_id:
            raise ValidationError(_('This line has no linked survey.'))
        return {
            'name': _('Edit khasra inputs / खसरा संपादन'),
            'type': 'ir.actions.act_window',
            'res_model': 'bhu.award.simulator.land.edit.wizard',
            'view_mode': 'form',
            'target': 'new',
            'context': {
