...
export class DynamicDashboardChart extends Component {
    ...
    setup() {
        ...
        this.processedHeight = Number(this.props.widget.height.replace('px', '')) - 100;
        this.processedWidth = Number(this.props.widget.width.replace('px', '')) - 50;
    ...
    async removeTile(ev){
        ...
        this.dialog.add(ConfirmationDialog, {
            ...
            confirm: async () => {
                await this.props.orm.call("dashboard.block", "make_kpi_inactive",[[],this.props.widget.id]);
                location.reload();
            },
            cancel: () => {},
        });
    ...
    renderChart(){
        if (this.props.widget.graph_type){
            const x_axis = this.props.widget.x_axis || [0]
            const y_axis = this.props.widget.y_axis || [0]
            const data = []
...
DynamicDashboardChart.template = xml`
    <div class="resize-drag block card"
        ...
        t-att-style="'height:'+this.props.widget.height+'; width:'+ this.props.widget.width+ '; top: '+ this.props.widget.translate_x +'; left: '+ this.props.widget.translate_y +';'"
        t-att-data-id="this.props.widget.id">
        <div class="card-body mt-1" id="in_ex_body_hide">
            <t t-if="this.props.widget.is_officer">
            ...
            <div class="row">
                <div class="col-md-12 chart_canvas" id="chart_canvas"
                    t-att-data-id="this.props.widget.id">
                    <canvas id="canvas" t-ref="chart" t-att-data-title="this.props.widget.name"     t-att-style="'height:' + this.processedHeight + 'px; width:' + this.processedWidth + 'px;'"/>
                </div>
            </div>
        </div>
    </div>
